...
fun SettingsScreen(
    ...
    GlassScaffold(
        ...
        Column(
            ...
            Text("连接设置", style = MaterialTheme.typography.titleMedium, color = OnDarkSurface)
            ...
            Text("首页接口显示", style = MaterialTheme.typography.titleMedium, color = OnDarkSurface)
            ...
            GlassCard(modifier = Modifier.fillMaxWidth()) {
                Column(
                    ...
                    if (uiState.isLoadingInterfaces) {
                        CircularProgressIndicator(modifier = Modifier.size(20.dp), strokeWidth = 2.dp, color = PrimaryTeal)
                    } else if (uiState.availableInterfaces.isEmpty()) {
                        Text("暂无可用接口，请先测试连接", style = MaterialTheme.typography.bodySmall, color = OnDarkSurfaceVariant)
                    } else {
            ...
            Text("危险区域", style = MaterialTheme.typography.titleMedium, color = StatusError)
            ...
            GlassCard(modifier = Modifier.fillMaxWidth()) {
                Column(
                    ...
                    Text("清除所有本地保存的 RouterOS 连接信息", style = MaterialTheme.typography.titleMedium, color = OnDarkSurface)
                    Text("此操作会删除当前保存的主机、端口、用户名、密码以及首页接口选择。", style = MaterialTheme.typography.bodySmall, color = OnDarkSurfaceVariant)
                    GlassButton(
                        text = "清除全部数据",
